...
This sample models a small but realistic platform repo with multiple component types, domains, and environments using arx component discovery.

`arx` v0.7.0 compiles the intent in `intent.yaml` into an immutable execution DAG of terraform, helm-values, and helm-chart jobs. Environment policy still lives in the intent, while each owned directory carries its own `component.yaml` manifest. The sample now demonstrates two of the newer arx operating patterns:
...
- pull requests compile a review-scoped plan from the explicit changed file list and upload `.artifacts/pr-plan.json` plus `.arx/component-tree.yaml`
- pull requests and pushes render a full `.artifacts/plan.json` and execute it with `arx run --gha`
...
To run the sample locally with `arx` on your `PATH`:
